...
LString SelOpNode::toString() const
{
    ...
    if (m_nmode == OP_NOT) {
        ...
        if (m_artarg.isEmpty())
            ...
            return LString::format("(%s) around [%s] %f", rval.c_str(),
                                   m_artarg.c_str(), m_dvalue);
...
{
    if (m_p1 != NULL) delete m_p1;
    if (m_p2 != NULL) delete m_p2;
}
...
{
    SelBinNode *pret = MB_NEW SelBinNode(m_p1->clone(), m_p2->clone(), m_nmode);
    ...
    switch (m_nmode) {
        case OP_AND:
            if (getNode1()->isSelected(pAtom)) {
                if (getNode2()->isSelected(pAtom)) return true;
            }
            return false;

        case OP_OR:
            if (getNode1()->isSelected(pAtom)) return true;
            if (getNode2()->isSelected(pAtom)) return true;
...
bool SelRefNode::isSelected(MolAtomPtr pAtom)
{
    if (m_pCachedSel.isnull()) {
        if (!resolveReference()) return false;  // ERROR!!
    }
    if (m_pCachedSel.isnull()) {
        // ERROR!!
        return false;
...
bool SelNamesNode::matches(const LString &nam) const
{
    ...
    for (; iter != endi; iter++) {
        if (iter->second == SNN_QSTR || iter->second == SNN_DQSTR) {
            // quoted string mode
            if (nam.equals(iter->first)) return true;
            ...
            if (rex.matchIgnoreCase(nam)) return true;
#else
            if (rex.match(nam)) return true;
#endif

        } else {
            // plain string mode
#ifdef STRMATCH_ICASE
            if (nam.equalsIgnoreCase(iter->first)) return true;
#else
            if (nam.equals(iter->first)) return true;
#endif
        }
...
bool SelNamesNode::isAtomSelected(const LString &aName, char altconf) const
{
    ...
    // check altconf
    if (altConfMatches(aName, altconf)) return true;
...
SelResidNode::SelResidNode(const SelResidNode &src) : m_list(src.m_list) {}
...
{
    if (m_pn != NULL) delete m_pn;
    if (m_pr != NULL) delete m_pr;
    if (m_pr2 != NULL) delete m_pr2;
}
...
SelSuperNode *SelTermNode::clone() const
{
    ...
    if (m_pn != NULL) pret->m_pn = (SelNamesNode *)m_pn->clone();
    if (m_pr != NULL) pret->m_pr = (SelRangesNode *)m_pr->clone();
    if (m_pr2 != NULL) pret->m_pr2 = (SelResidNode *)m_pr2->clone();
...
{
    if (m_pChains != NULL) delete m_pChains;
    if (m_pResids != NULL) delete m_pResids;
    if (m_pAtoms != NULL) delete m_pAtoms;
}
...
SelSuperNode *SelHierNode::clone() const
{
    ...
    if (m_pChains != NULL) pret->m_pChains = (SelNamesNode *)m_pChains->clone();
    if (m_pResids != NULL) pret->m_pResids = (SelResidNode *)m_pResids->clone();
    if (m_pAtoms != NULL) pret->m_pAtoms = (SelNamesNode *)m_pAtoms->clone();
...
bool SelHierNode::isSelected(MolAtomPtr pAtom)
{
    ...
    if (pCh != NULL) {
        ...
        if (!pCh->matches(sym)) return false;
    }
    ...
    if (pRi != NULL) {
        ...
        if (!pRi->isResidSelected(ind)) return false;
    }
    ...
    if (pAt != NULL) {
        sym = pAtom->getName();
        if (!pAt->isAtomSelected(sym, pAtom->getConfID())) return false;
    }
...
SelCompNode::SelCompNode() : m_nmode(0), m_ncompop(0), m_dvalue(0.0) {}
...
SelPropNode::SelPropNode() : m_nmode(0) {}

SelPropNode::SelPropNode(const SelPropNode &r)
    : m_nmode(r.m_nmode), m_propname(r.m_propname), m_propvalue(r.m_propvalue)
{
}

SelPropNode::SelPropNode(int mode, const char *name, const char *value)
    : m_nmode(mode), m_propname(name), m_propvalue(value)
{
}
...
LString SelPropNode::toString() const
{
    ...
    return LString::format("%s %s=%s", mode.c_str(), m_propname.c_str(),
                           m_propvalue.c_str());
}
