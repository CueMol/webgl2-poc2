...
public:
    ...
    /// default ctor
    SelOpNode() : m_p(NULL), m_nmode(OP_NOT), m_dvalue(0.0) {}
...
private:
    ...
    SelOpNode(const SelOpNode &arg)
        : m_p(arg.m_p),
          m_nmode(arg.m_nmode),
          m_dvalue(arg.m_dvalue),
          m_artarg(arg.m_artarg)
    {
    }

public:
    /// specific ctor with opeator mode
    SelOpNode(SelSuperNode *p, int mode) : m_p(p), m_nmode(mode), m_dvalue(0.0) {}

    virtual ~SelOpNode();

    SelSuperNode *getNode()
    {
        return m_p;
    }
    int getMode() const
    ...
    LString getAroundTarget() const
    {
        return m_artarg;
    }
    void setAroundTarget(const char *s)
    {
        m_artarg = s;
    }
...
public:
    ...
    SelBinNode() : m_p1(NULL), m_p2(NULL), m_nmode(OP_AND) {}
    ...
    SelSuperNode *getNode1()
    {
        return m_p1;
    }
    SelSuperNode *getNode2()
    {
        return m_p2;
    }
    int getMode() const
...
public:
    SelAllNode(bool fall = true) : m_fall(fall) {}
...
public:
    ...
    SelRefNode(const SelRefNode &arg) : m_name(arg.m_name) {}

    SelRefNode(const char *name) : m_name(name) {}
...
public:
    ...
    SelRangesNode(int n1, int n2)
    {
        m_list.append(n1, n2 + 1);
    }
    ...
    const RangeSet<int> &getRangeSet() const
    {
        return m_list;
    }
...
public:
    ...
    const RangeSet<ResidIndex> &getRangeSet() const
    {
        return m_list;
    }
...
public:
    SelHierNode() : m_pChains(NULL), m_pResids(NULL), m_pAtoms(NULL) {}
    ...
    SelNamesNode *getChains()
    {
        return m_pChains;
    }
    SelResidNode *getResids()
    {
        return m_pResids;
    }
    SelNamesNode *getAtoms()
    {
        return m_pAtoms;
    }
...
public:
    ...
    SelTermNode() : m_pn(NULL), m_pr(NULL), m_pr2(NULL), m_nmode(ELEMENT) {}
    ...
    SelRangesNode *getRangesNode()
    {
        return m_pr;
    }
    SelResidNode *getResidNode()
    {
        return m_pr2;
    }
    SelNamesNode *getNamesNode()
    {
        return m_pn;
    }
    int getMode() const
...
public:
    enum { COMP_BFAC, COMP_OCC, COMP_EQ, COMP_GT, COMP_LT };
