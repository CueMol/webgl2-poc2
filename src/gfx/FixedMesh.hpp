...
public:
FixedMesh() : m_nVerts(0), m_nFaces(0) {}
    ...
    inline void setVertex(int i, float x, float y, float z, float nx, float ny,
                          float nz)
    {
        m_verts[i * 3 + 0] = x;
        m_verts[i * 3 + 1] = y;
    ...
    {
        return Vector4D(m_verts[i * 3 + 0], m_verts[i * 3 + 1], m_verts[i * 3 + 2]);
    }
    ...
    {
        return Vector4D(m_norms[i * 3 + 0], m_norms[i * 3 + 1], m_norms[i * 3 + 2]);
    }
    ...
    bool convRGBAByteCols(quint8 *pcols, int nsize, int defalpha = 255,
                          qlib::uid_t nSceneID = qlib::invalid_uid) const;
};
