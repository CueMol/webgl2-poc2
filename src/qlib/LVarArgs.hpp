...
{
public:
    LVarArgs(int nsize = 0) : m_nSize(nsize), m_pthis(NULL)
    {
        if (nsize > 0)
    ...
    void checkArgSize(int n) const
    {
        if (m_nSize != n) {
            LString msg =
                LString::format("Argument size mismatch %d != %d", n, m_nSize);
    ...
    _Type *getThisPtr() const
    {
        ...
        if (pret == NULL) {
            MB_DPRINTLN("cannot dyncast %p(%s) to %s", m_pthis, typeid(*m_pthis).name(),
                        typeid(_Type).name());
            return NULL;
        }
        return pret;
