...
bool PipeStreamImpl::ready()
{
    ...
    if (m_feof) return false;  // channel was closed
...
int PipeStreamImpl::read()
{
    ...
    while (!m_feof && m_data.size() <= 0) m_cond.wait(lk);

    if (m_feof && m_data.size() <= 0) return -1;
