...
public:
    ...
    virtual int skip(int n)
    {
        ...
        if (res < 0) MB_THROW(IOException, "Cannot seek file ptr");
        // MB_DPRINTLN("fpos: %d", ::ftell(m_fp));
        return n;
    }
...
FileInStream::FileInStream() : m_pimpl(MB_NEW PosixFIOImpl())
{
    // setImpl(m_pimpl);
...
FileOutStream::FileOutStream() : m_pimpl(MB_NEW PosixFIOImpl())
{
    //  setImpl(m_pimpl);
}

/** copy ctor */
FileOutStream::FileOutStream(const FileOutStream &r) : m_pimpl(r.m_pimpl)
{
    //  setImpl(m_pimpl);
}
