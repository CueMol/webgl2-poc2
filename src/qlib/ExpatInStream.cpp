...
ExpatInStream::ExpatInStream(InStream &r) : super_t(r)
{
    // m_parser =  XML_ParserCreate(NULL);
    m_parser = XML_ParserCreate("UTF-8");
    XML_SetUserData(m_parser, this);
    XML_SetElementHandler(m_parser, ExpatInStream::sStartElem, ExpatInStream::sEndElem);

    XML_SetCharacterDataHandler(m_parser, sCharData);
...
void ExpatInStream::setError(const LString &msg)
{
    m_errorMsg = msg;
    m_errorMsg.append(
        LString::format(" at line %d", XML_GetCurrentLineNumber(m_parser)));
...
void ExpatInStream::parse()
{
    ...
    stdfs::path srcpath(s.c_str());
    LString base_dir = srcpath.parent_path().string();

    //   boostfs::path srcpath(s.c_str());
    ...
    if (!base_dir.isEmpty()) setBaseURI(base_dir);
...
// static
void ExpatInStream::sCharData(void *userData, const XML_Char *s, int len)
{
    LString sbuf(s, len);
...
// static
int ExpatInStream::sExtEntity(XML_Parser parser, const XML_Char *context,
                              ...
                              const XML_Char *publicId)
{
    ...
    if (systemId == NULL || LChar::length(systemId) == 0) return 1;

    ExpatInStream *pReader = (ExpatInStream *)XML_GetUserData(parser);

    LString uri;
    if (base != NULL) uri = LString(base);
    uri += systemId;
    ...
    try {
        ...
        LOG_DPRINTLN("Error in handling external entity %s: \"%s\"", systemId,
                     e.getMsg().c_str());
        return 0;
    }
