...
if (BUILD_NODEJS_BINDINGS)
  SET(MCWG_JS_CMD "${MCWRAPGEN} ${MCWG_INCLUDES} -m js")
  message(STATUS "MCWG_JS_CMD: ${MCWG_JS_CMD}")
endif ()
# Javascript (Emscripten)
if (BUILD_EMSCRIPTEN_BINDINGS)
  SET(MCWG_JS_CMD "${MCWRAPGEN} ${MCWG_INCLUDES} -m jsem")
  message(STATUS "MCWG_JS_CMD: ${MCWG_JS_CMD}")
endif ()
...
macro(MCWRAPGEN_CLASS _target_sources)
  foreach(_current_file ${ARGN})
    ...
    if (${BUILD_NODEJS_BINDINGS} OR ${BUILD_EMSCRIPTEN_BINDINGS})
      if (WIN32)
        SET(_out_js_dir "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/javascript/wrappers")
      else ()
      ...
      add_custom_command(
	    OUTPUT ${_out_js_file}
	    COMMAND ${_mcwg_js_command} -jsdir ${_out_js_dir} ${_abs_file}
        DEPENDS ${_abs_file}
	    )
      list(APPEND MCWG_JS_WRAPPERS ${_out_js_file})
      message(STATUS "_out_js_file: ${_out_js_file}")
    endif ()
...
macro(MCWRAPGEN_JS_WRAPPERS _target)
  if (${BUILD_NODEJS_BINDINGS} OR ${BUILD_EMSCRIPTEN_BINDINGS})
    add_custom_target(${_target}_generate_js_wrappers DEPENDS ${MCWG_JS_WRAPPERS})
    add_dependencies(${_target} ${_target}_generate_js_wrappers)
    install(FILES ${MCWG_JS_WRAPPERS} DESTINATION data/javascript/cuemol/wrappers)
  endif ()
endmacro()
