...
void SceneXMLWriter::write()
{
    ...
    // Update the SourcePath/BasePath
    {
        ...
        // convert it to the absolute path name using the boostfs::current_path() (i.e.
        // pwd)
        auto curpath = stdfs::current_path();
        LString localfile = qlib::makeAbsolutePath(getPath(), curpath.string());

        //     // If getPath() points to a relative path,
        //     // convert it to the absolute path name using the boostfs::current_path()
        //     (i.e. pwd) boostfs::path curpath = boostfs::current_path();
        // #if (BOOST_FILESYSTEM_VERSION==2)
        //     LString localfile = qlib::makeAbsolutePath(getPath(),
        //     curpath.file_string());
        // #else
        //     LString localfile = qlib::makeAbsolutePath(getPath(), curpath.string());
        // #endif
    ...
    if (m_bForceEmbedAll) m_pClient->forceEmbed();
...
void SceneXMLWriter::procDataChunks(qlib::LDom2OutStream &oos, qlib::LDom2Node *pNode)
{
    ...
    for (; iter != eiter; ++iter) {
        ...
        if (pChNode != NULL) procDataChunks(oos, pChNode);
    }
    ...
    if (pCnt == NULL || !pCnt->isDataSrcWritable()) return;
...
qlib::LScrSp<qlib::LByteArray> SceneXMLWriter::toByteArray(
    const qlib::LScrSp<qlib::LScrObjBase> &pSObj, const LString &type_ovwr)
{
    // Setup streams
        ...
        if (!pRend.isnull()) {
            ...
            if (!pCam.isnull()) {
                ...
                if (!pStySet.isnull()) {
                    ...
                    MB_THROW(qlib::InvalidCastException,
                             "toXML: obj is not obj or renderer or camera");
...
qlib::LByteArrayPtr SceneXMLWriter::rendArrayToByteArray(
    const std::list<RendererPtr> &rendary, const LString &grpname)
{
    // Setup streams
    qlib::StrOutStream fos;
    qlib::LDom2OutStream oos(fos);
