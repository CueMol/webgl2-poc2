...
void SceneXMLReader::read()
{
    ...
    // convert it to the absolute path name using the boostfs::current_path() (i.e. pwd)
    stdfs::path curpath = stdfs::current_path();
    LString localfile = qlib::makeAbsolutePath(getPath(), curpath.string());

    //     boostfs::path curpath = boostfs::current_path();
...
#endif
{
    for (;;) {
        try {
            ...
            if (pCnt == NULL) {
                LOG_DPRINTLN("SceneXMLReader> data container for chunk %s not found.",
                             chunkid.c_str());
                return;
            }
...
#endif
{
    ...
    for (; iter != eiter; ++iter) {
        ...
        if (pChNode != NULL) procDataSrcLoad(ois, pChNode);
    }
    ...
    if (srctype.isEmpty()) {
        ...
        LOG_DPRINTLN("SceneXML> src %s: srctype is not defined. (ignored)",
                     src.c_str());
        return;
    }
    ...
    if (src.startsWith("datachunk:") && src.length() == 15) {
        ...
        MB_DPRINTLN("Data chunk found %s, %s ", src.c_str(), srctype.c_str());
    ...
    // (In the qsc file, there are two alternative sources. However, only one src is
    // actually loaded.) This is required to keep consistency between the "src prop" and
    // "actual data src".
    if (bAlt) {
        ...
        if (!qlib::isAbsolutePath(src)) src = qlib::makeAbsolutePath(src, basedir);
                                            ...
                                            LString &grpName)
{
    ...
    for (pNode->firstChild(); pNode->hasMoreChild(); pNode->nextChild()) {
        ...
        if (!tag.equals("renderer") || type_name.isEmpty()) continue;
