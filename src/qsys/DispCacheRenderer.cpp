...
DispCacheRenderer::DispCacheRenderer() : super_t()
{
    m_bShaderAlpha = View::hasVS();
}

DispCacheRenderer::DispCacheRenderer(const DispCacheRenderer &r) : super_t(r) {}
...
void DispCacheRenderer::objectChanged(ObjectEvent &ev)
{
    if (ev.getType() == ObjectEvent::OBE_CHANGED) {
        ...
        invalidateDisplayCache();
        MB_DPRINTLN(
            "XXX DispCacheRenderer::objectChanged> CALL invalidateHittestCache()");
        invalidateHittestCache();
...
{
    if (!m_bShaderAlpha) {
        if (ev.getName().equals("alpha") || ev.getName().equals("material"))
            invalidateDisplayCache();
    }
...
void DispCacheRenderer::sceneChanged(SceneEvent &ev)
{
    ...
    if (ev.getType() == SceneEvent::SCE_SCENE_PROPCHG) {
        if (ev.getDescr() == "icc_filename" || ev.getDescr() == "use_colproof" ||
...
DispListCacheImpl::DispListCacheImpl() : m_pdl(NULL), m_phl(NULL) {}
...
{
    if (m_pdl != NULL) delete m_pdl;
    m_pdl = NULL;

    if (m_phl != NULL) delete m_phl;
    m_phl = NULL;
}
