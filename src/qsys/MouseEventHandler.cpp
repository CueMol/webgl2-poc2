...
bool MouseEventHandler::buttonUp(InDevEvent &ev)
{
    if (m_nState == DRAG_CHECK) {
        ...
        MB_DPRINTLN("prev %f currt %f del_t %f", double(m_prevClickTime), double(currt),
                    double(del_t));

        if (del_t < qlib::time_value(DBLCLICK_TIME)) {
            MB_DPRINTLN("del_t %f < DBLCLICK_TIME %f", double(del_t),
            ...
            if (ev.isLButtonOn())
                ...
                LOG_DPRINTLN("buttonUp dblclk> ERROR: unknown btnclick (%d)",
                             ev.getModifier());
                ev.setType(InDevEvent::INDEV_LBTN_DBLCLICK);
            }
        } else {
            ...
            if (ev.isLButtonOn())
                ...
                LOG_DPRINTLN("buttonUp> ERROR: unknown btnclick (%d)",
                             ev.getModifier());
                ev.setType(InDevEvent::INDEV_LBTN_CLICK);
            }
        }
