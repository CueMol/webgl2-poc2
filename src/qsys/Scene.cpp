...
void Scene::init()
{
    ...
    if (m_pQscOpts != NULL) delete m_pQscOpts;
    m_pQscOpts = NULL;
}
...
#ifndef NO_SCRIPT
    if (m_pInterp != NULL) m_pInterp->setScriptPath("scene_base_path", getBasePath());
#endif
}
...
ObjectPtr Scene::getObjectByName(const LString &name) const
{
    BOOST_FOREACH (const data_t::value_type &i, m_data) {
        ...
        if (i.second->getName().equals(name)) return i.second;
    }
    return ObjectPtr();
}
...
bool Scene::registerObjectImpl(ObjectPtr pObj)
{
...

    {
        // Record undo/redo info
        UndoManager *pUM = getUndoMgr();
        if (!pUM->isOK()) return true;  // undo manager is disabled now.
...
bool Scene::removeRendCache(RendererPtr prend)
{
    ...
    if (i == m_rendtab.end()) return false;
...
RendererPtr Scene::getRendByName(const LString &nm) const
{
    ...
    for (; riter != reiter; ++riter) {
        RendererPtr prend = riter->second;
        if (nm.equals(prend->getName())) return prend;
    }
...
static LString makeSectionName(ObjectPtr pObj, RendererPtr prend)
{
    /*
...
void Scene::display(DisplayContext *pdc)
{
...

    {
        ...
        for (; i != ie; ++i) {
            ...
            if (pObj.isnull() || !pObj->isVisible()) continue;

            if (!pRend->isVisible()) continue;
    ...
    // Display 2D labels (UI elements /w depth)
    {
        ...
        for (; i != ie; ++i) {
            RendererPtr prend = i->second;
            ObjectPtr pobj = prend->getClientObj();
            if (!pobj.isnull() && pobj->isVisible() && prend->isVisible()) {
...
ViewPtr Scene::getView(qlib::uid_t uid) const
{
    ...
    if (i == m_viewtab.end()) return ViewPtr();
...
bool Scene::destroyView(qlib::uid_t uid)
{
    ...
    if (i == m_viewtab.end()) return false;
    ...
    if (m_viewtab.size() == 1) {
        ...
        for (; riter != m_rendtab.end(); ++riter) {
            RendererPtr pRend = riter->second;
            if (!pRend.isnull()) pRend->unloading();
        }

        data_t::const_iterator oiter = m_data.begin();
        for (; oiter != m_data.end(); ++oiter) {
            ObjectPtr pObj = oiter->second;
            if (!pObj.isnull()) pObj->unloading();
        }
    }
...
{
    LString rval;
    if (m_viewtab.empty()) return rval;
...
bool Scene::isModified() const
{
    ...
    if (m_undomgr.getUndoSize() == 0 && !pSM->isModified(getUID()))
        return false;
    else
        return true;
}
...
bool Scene::isJustCreated() const
{
    ...
    if (getObjectCount() == 0 && getCameraCount() == 0) {
        // getCameraCount()==1 &&
...
void Scene::propChanged(qlib::LPropEvent &ev)
{
    ...
    if (ev.getName() == "icc_filename" || ev.getName() == "use_colproof" ||
...
LString Scene::getSceneDataJSON(bool bGroup) const
{
    ...
    BOOST_FOREACH (qlib::uid_t objid, obj_uids) {
        ...
        rval += LString("\"ui_collapsed\": ") +
                LString::fromBool(obj->isUICollapsed()) + ", ";
...
CameraPtr Scene::getCameraRef(const LString &name) const
{
    ...
    if (i == m_camtab.end()) return CameraPtr();
    return i->second;
}
...
{
    camtab_t::const_iterator i = m_camtab.find(name);
    if (i == m_camtab.end()) return false;
    return true;
}
...
bool Scene::saveCameraTo(const LString &aName, const LString &aLocalFile) const
{
    ...
    if (aName.equals("__current")) return true;
...
void Scene::setCamToViewAnim(qlib::uid_t viewid, const LString &name, bool bAnim)
{
    ...
    if (viewid == qlib::invalid_uid) {
        BOOST_FOREACH (const viewtab_t::value_type &i, m_viewtab) {
            ...
            if (id != qlib::invalid_uid) Scene::setCamToViewAnim(id, name, bAnim);
        }
        return;
    }
...
{
    if (m_undomgr.getUndoSize() == 0 && m_undomgr.getRedoSize() == 0)
...
void Scene::writeTo2(qlib::LDom2Node *pNode) const
{
    ...
    if (m_pAnimMgr->getSize() > 0 || m_pAnimMgr->getLength() > 0) {
        qlib::LDom2Node *pChNode = pNode->appendChild();
...
void Scene::stylesWriteTo(qlib::LDom2Node *pNode) const
{
    ...
    BOOST_REVERSE_FOREACH(StyleList::value_type pSet, *pSL)
    {
        ...
        if (src.isEmpty()) {
            ...
            // Reset the modified flag (because the contents was synchronized to the
            // scene file)
            pSet->setModified(false);
        } else {
            // External style reference is serialized as external reference node.
            // setup src and altsrc attributes
            std::pair<LString, LString> res = setPathsToNode(src, LString(), pChNode);
            ...
            if (bReadOnly) pChNode->appendStrAttr("readonly", "true");

            if (!bReadOnly) {
                // Always write style to the external file
                // TO DO: update external source, only when the loaded style is
                // modified.
                LString abspath = qlib::makeAbsolutePath(src, basedir);
                LOG_DPRINTLN("Scene> style %s is saved to external src %s", id.c_str(),
                             abspath.c_str());
                qlib::uid_t nStyleSetID = pSet->getUID();
                bool bRes =
                    m_pStyleMgr->saveStyleSetToFile(nScopeID, nStyleSetID, abspath);
                if (!bRes) {
                    LOG_DPRINTLN("Scene> write external style file %s failed.",
                                 abspath.c_str());
                }
                // StyleMgr.saveStyleSetToFile() method always reset the modified flag
                // if save is succeeded.
            }
        }
...
void Scene::cameraReadFrom(qlib::LDom2Node *pNode)
{
    ...
    if (!pNode->isChildrenConsumed()) {
        ...
        // LOG_DPRINTLN("Scene::readFrom2> Warning: some Camera nodes are not
        // consumed"); pNode->dump();
    }
...
void Scene::stylesReadFrom(qlib::LDom2Node *pNode)
{
    ...
    if (!src.isEmpty()) {
        ...
        bool bAlt;
        LString abs_path =
            qlib::LDataSrcContainer::selectSrcAltSrc(src, alt_src, base_path, bAlt);
    ...
    try {
        ...
        for (pNode->firstChild(); pNode->hasMoreChild(); pNode->nextChild()) {
            ...
            if (tag.equals("object") && !type_name.isEmpty()) {
                ...
                if (value.isEmpty() && !contents.isEmpty()) value = contents;

#ifndef NO_SCRIPT
                // TO DO: set file/lineno info for eval()
                // TO DO: save the original script for serialization
                if (m_pInterp != NULL) m_pInterp->eval(value);
#endif
